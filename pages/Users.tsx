...
>>>>>>> b1fbf46c003fb4d099b7af607824fabd37368f0f
    ...
    const validate = (data: typeof formData) => {
        ...
        if (data.mobile && !/^01[0125]\d{8}$/.test(data.mobile)) {
            newErrors.mobile = 'رقم المحمول يجب أن يكون 11 رقمًا ويبدأ بـ 010 أو 011 أو 012 أو 015.';
        }
         if (data.mobile && existingUsers.some(u => u.mobile === data.mobile)) {
            newErrors.mobile = 'رقم المحمول هذا موجود بالفعل.';
        }
        if (data.username && existingUsers.some(u => u.username.toLowerCase() === data.username.toLowerCase())) {
    ...
    const isFormInvalid = useMemo(() => {
        return !formData.name || !formData.mobile || !formData.username || !formData.password || !formData.role || Object.values(errors).some(e => !!e);
    }, [formData, errors]);
...
<<<<<<< HEAD
            ...
            <div>
                <input type="text" name="username" value={formData.username} onChange={handleChange} placeholder="اسم المستخدم" required className={inputClasses} />
                {errors.username && formData.username && <p className="text-red-500 text-sm mt-1">{errors.username}</p>}
            </div>
            <input type="password" name="password" value={formData.password} onChange={handleChange} placeholder="كلمة المرور" required className={inputClasses} />
...
>>>>>>> b1fbf46c003fb4d099b7af607824fabd37368f0f
            ...
            <div className="flex justify-end space-x-2 space-x-reverse pt-4">
                ...
                <Button type="submit" variant="primary" disabled={isFormInvalid}>إضافة مستخدم</Button>
            </div>
        </form>
    );
};
...
>>>>>>> b1fbf46c003fb4d099b7af607824fabd37368f0f
    ...
    const validate = (data: typeof formData) => {
        ...
        if (data.mobile && existingUsers.some(u => u.id !== userToEdit.id && u.mobile === data.mobile)) {
    ...
    }, [formData, userToEdit, existingUsers]);
    ...
    const isFormInvalid = useMemo(() => {
        return !formData.name || !formData.mobile || !formData.role || Object.values(errors).some(e => !!e);
    }, [formData, errors]);
...
>>>>>>> b1fbf46c003fb4d099b7af607824fabd37368f0f
            ...
            <div className="flex justify-end space-x-2 space-x-reverse pt-4">
                <Button type="button" variant="secondary" onClick={onClose}>إلغاء</Button>
                <Button type="submit" variant="primary" disabled={isFormInvalid}>تعديل</Button>
            </div>
        </form>
    );
};
